...
class DBHelper:
    def __init__(self, db_sesison: Session):
        self.db_session = db_sesison
    ...
    def get_iser_by_name(self, name: str):
        return self.db_session.query(UserDBModel).filter(UserDBModel.name == name).first()
    ...
    def cleanup_test_data_user(self, objects_to_delete: list):
    ...
    def create_test_movies(self, movie_data: dict) -> MovieDBModel:
    ...
    def cleanup_test_data_movie(self, objects_to_delete: list):
        for obj in objects_to_delete:
            if obj:
                self.db_session.delete(obj)
        self.db_session.commit()